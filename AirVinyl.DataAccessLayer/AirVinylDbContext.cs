...
namespace AirVinyl.DataAccessLayer
{
    public class AirVinylDbContext : DbContext
    {
        ...
        {
            // we need unchanged value as well - the object dictionary isn't tracked
            // by EF, this changes to it will not be tracked either
            ...
            for (int i = 0; i < modifiedOrAddedVinylRecords.Count; i++)
            {
                ...
                // remove the current dynamic property references
                // this does not remove the record itself
                vinylRecord.Entity.DynamicVinylRecordProperties.Clear();
                foreach (var dynamicPropertyToSave in dynamicProperties)
                {
                    // first, find & delete the actual record itself if it exists
